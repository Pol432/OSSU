...
fun preprocess_prog e =
    case e of
       LineSegment (x1,y1,x2,y2) => let val xs = if (x1 > x2)
						 then (x2,x1) else (x1,x2)
					val ys = if (y1 > y2)
                                                 then (y1,y2) else (y2,y1) 
				     in  if (real_close_point(x1,y1)(x2,y2))
					 then Point(x1,y1)
					 else if (real_close(x1,(#2 xs)))
					 then LineSegment((#1 xs),(#2 ys), (#2 xs), (#1 ys))
					 else LineSegment((#1 xs),y1, (#2 xs), y2) end 
      | _ => e
