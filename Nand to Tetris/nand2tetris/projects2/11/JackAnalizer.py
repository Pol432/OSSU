...
def main():
    ...
    if os.path.isfile(input_path):
        ...
        jackFilename = os.path.splitext(input_path)[0]
        jackFile = open(input_path, "r")
        ...
        for file_name in files:
            if file_name.endswith(".jack"):
                jackFile = open(os.path.join(input_path, file_name), "r")
                jackFilename = file_name.split(".")[0]
                tokFile = os.path.join(input_path, jackFilename + "Token" + ".xml")
                
                tokenizer(jackFile, tokFile)
                Compiler(tokFile, os.path.join(input_path, jackFilename + ".xml"))
                os.remove(tokFile)
    else:
...
def tokenizer(file, outfile):
    global xmlToken
    xmlToken = open(outfile, "w")
